...
<div class=" m-24 grid  grid-cols-1">
     ...
     <div
       v-else
        class="p-8"
     >
     <div class="max-w-sm rounded-3xl overflow-hidden shadow-lg ml-96 ">

      <div class="px-6 py-4">
        <div class=" text-red-500 font-bold text-2xl text-center">{{ result.character.name }}</div>
      </div>
          <div class="relative">
            <div 
            :class="{
            ...
            class="inline-block  rounded-lg px-3 py-0 text-lg font-bold  mr-2 mb-2 absolute right-2 top-2 text-white">{{ result.character.status}}</div>
    ...
    > 
         ...
         <RouterLink
           to="/episode"
         class="text-blue-500 text-3xl font-bold hover:text- hover:underline mt-10">
          Episode <small class="text-gray-500 text-sm"> ({{result.character.name}} participate)</small>
       </RouterLink>
       <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8 p-3 mt-6">
          <RouterLink  
          :to="`/episode/${episode.id}`"
          v-for="episode in result.character.episode" :key="episode.id" class=" p-5 py-2">  
            
            <p class="text-3xl text-center my-4 font-bold text-blue-950 hover:underline hover:text-blue-600  p-3 ">
                 {{ episode.name }}
            </p>
            <p class="text-2xl ml-12 p-3 ">
